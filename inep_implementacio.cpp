...
int main() {
	...
	while (true) {
		...
		case 5:
			return 0;
		default:
			cout << "Selecciona una opcio v�lida amb les tecles de l'1 al 5" << endl;
		}
	}
	return 0;
}
...
/*int main() {
	try {
		...
		// 1.- es crea un pqxx::work amb la connexi� que hem creat
		pqxx::work txn(conn);
		// 2.- s'executa una comanda en SQL que correspon a la consulta
		pqxx::result result = txn.exec("SELECT * FROM musics");
		// si a l'executar us d�na el missatge que no troba la taula, potser necessiteu posar :
		// "SELECT * FROM public.musics" o ""SELECT * FROM public.\"MUSICS\"
		// podeu confirmar la sintaxi de la comanda en el pgAdmin, sobre la taula escollir la opci� Scripts\SELECT Script
		// Per mostrar el resultat de la consulta, hem de rec�rrer les files (primer for) ...
		cout << "--------------------------------------------" << endl;
		cout << "Opcio de recorrer el resultat amb dos loops" << endl;
		cout << "-------------------------------------------" << endl;
		for (const auto& row : result) {
			// ... i per cada fila, es recorren els camps (l'ordre �s el que es veu a les columnes al pgAdmin
			for (const auto& field : row) {
				cout << field.name() << ": " << field.c_str() << "\t";
			}
			cout << endl;
		}
		// tamb� es poden rec�rrer els resultats com si fosin una matriu de files i columnes(amb indexos comen�ant per 0)
		cout << "---------------------------------------------" << endl;
		...
		// Finalment, s'ha de confirmar la transacci�n
		txn.commit();
		// La connexi� es tanca autom�ticament al sortir del try
	}
